...
class firstWindowUI(QDialog):
    def __init__(self):
        super().__init__()
        loadUi("enternum.ui", self)
        self.setWindowTitle("Tower of Hanoi GUI")
        self.setFixedSize(self.width(),self.height())
        self.runclick.clicked.connect(self.goToMainWindow)
        self.show()
        
    def goToMainWindow(self):
        self.close()
        input_txt = int(self.enternumber.text())
        mainWindow = mainWindowUI(num_of_disk=input_txt)
        mainWindow.show()
        

class mainWindowUI(QDialog):
    def __init__(self,num_of_disk):
        self.disks_lbl = {}
        self.check_col = {
            'A' : 0,
            'B' : 0,
        ...
        self.TowerOfHanoi(self.num_of_disk,'A','C','B')
        ...
        self.exitbtn.clicked.connect(self.goToFirstPage)
        self.pushButton_2.setHidden(True)
        self.check_col["A"] = self.num_of_disk
        self.show()   

    def goToFirstPage(self):
        self.close()
        self.firstWindow = firstWindowUI()
        self.firstWindow.show()  
        

    def create_disks(self,instance):
        color_range = range(256)
        biggest = self.num_of_disk
        count = 0
        biggest_y = 480
        lowest_x = 100
        width = 240
    ...
    def goToNextRound(self,Node:Node):
        ...
        if(self.movement == (2**self.num_of_disk)-1):
